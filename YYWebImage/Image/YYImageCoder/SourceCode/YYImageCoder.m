...
static BOOL YYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    //能进入下面就代表用vImageConvert_AnyToAny转换失败
    ...
    //根据之前获取到的contextBitmapInfo创建一个新的位图上下文
    ...
    if (hasAlpha && !alphaPremultiplied) {
        ...
        if (alphaFirst && byteOrderNormal) {
            //如果alpha通道排在前面 而且是正常的字节顺序 则调用下面的方法
            error = vImageUnpremultiplyData_ARGB8888(&tmpSrc, dest, kvImageNoFlags);
        } else {
            //如果alpha通道排在前面 而且小于32位的字节顺序 则调用下面的方法
...
CGImageRef YYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        //获取alpha通道信息 然后对比位图alpha通过信息
        CGImageAlphaInfo alphaInfo = CGImageGetAlphaInfo(imageRef) & kCGBitmapAlphaInfoMask;
        BOOL hasAlpha = NO;
        //如果alphaInfo 是 预乘型的RGBA 或者 预乘型ARGB 或者非预乘型的RGBA 或者 非预乘型ARGB
        //其实就判断是否有alpah通道
        ...
        //假如有alpha通道 则将添加kCGImageAlphaPremultipliedFirst 否则 则去除alpha通道 即添加kCGImageAlphaNoneSkipFirst
        bitmapInfo |= hasAlpha ? kCGImageAlphaPremultipliedFirst : kCGImageAlphaNoneSkipFirst;
        //构建图片上下文 这里就跟sdwebimage一样
        ...
        //bitsPerComponent 是指位图每像素中每个颜色占用的位空间 8比特
        size_t bitsPerComponent = CGImageGetBitsPerComponent(imageRef);
        //bitsPerPixel 是指每个像素的字节数 即通道数乘以8比特
        size_t bitsPerPixel = CGImageGetBitsPerPixel(imageRef);
        //bytesPerRow 位图的每一行包含多少字节 width * bitsPerComponent
        ...
        //指定dataProvider获取一份图片数据的copy
        CFDataRef data = CGDataProviderCopyData(dataProvider); // decode
        if (!data) return NULL;
        //通过这份图片数据copy 创建一个新的图片数据提供者
...
CGImageRef YYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    ...
    //根据之前的设置 获取其改变后的data 创建CGDataProvider实例
    ...
    //能进入下面就是说destBitmapInfo 是kCGBitmapByteOrder32Little 而且alpha通道是在最后的
...
CGImageRef YYCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    ...
    //如果移动后不是跟原本一样 就要进入下面的方法重新创建一个CGImageRef
...
- (BOOL)updateData:(NSData *)data final:(BOOL)final {
    ...
    //根据不同图片类型 对data进行不同方式的解码 并通过_frames数组持有每一帧的信息 CGImageSourceRef结构体对象_source获取到图片的图片资源
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    //因为是更新数据 所有假如新数据的长度 少于 旧数据的长度 则直接返回no
    ...
    //判断是否已经获取过类型 若有 则进入下面的判断
    if (_sourceTypeDetected) {
        //若已保存类型与现在的类型不符合 则返回no 否则 则进入else的判断
        if (_type != type) {
            return NO;
        } else {
            [self _updateSource];
...
- (YYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    ...
    //如果图片类型不是YYImageTypeICO 并且decodeForDisplay为yes 则进入下面的判断
    if (_type != YYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    //若不需要混合
    if (!_needBlend) {
        //直接通过类型 判断是否需要绘制在画布上 若不需要 直接调用CGBitmapContextCreate 解压缩成位图
        ...
        if (decodeForDisplay && !decoded) {
            //直接再调用一次YYCGImageCreateDecodedCopy解码
        ...
        //通过imageRef（有可能是已经解压缩 或者 没解压缩成功的） 生成uiimage类的实例
        UIImage *image = [UIImage imageWithCGImage:imageRef scale:_scale orientation:_orientation];
        CFRelease(imageRef);
        if (!image) return nil;
        //增加了一个自定义属性isDecodedForDisplay 判断该是否已经解压缩成功的
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        //如果不是前一帧混合 则需要判断frame.blendFromIndex == frame.index
        ...
        if (frame.blendFromIndex == frame.index) {
            //如果是从当前帧开始 则将_blendCanvas上下文的内容绘制为当前帧的未混合的内容
            ...
            if (frame.dispose == YYImageDisposeBackground) {
                //如果frame.dispose == YYImageDisposeBackground 则将_blendCanvas上下文清除内容
                CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
            }
            _blendFrameIndex = index;
        } else { // canvas is not ready
            //如果不是 就需要重blendFromIndex之后的每一帧开始 即所有帧都需要调用_newBlendedImageWithFrame
...
- (void)_updateSourceWebP {
    //若enable了WEBP类型的解码 则会引入相关webp类型的解码框架
#if YYIMAGE_WEBP_ENABLED
    _width = 0;
    _height = 0;
    _loopCount = 0;
    //假如有_webpSource 释放_webpSource的空间
    if (_webpSource) WebPDemuxDelete(_webpSource);
    _webpSource = NULL;
    //等待_framesLock的信号
    dispatch_semaphore_wait(_framesLock, DISPATCH_TIME_FOREVER);
    ...
    //我们可以使用 WebPIDecoder 去逐步解码 webp 数据；
    //但是在解码后只能返回一个空图片，并不能像 jpeg 图那样显示一部分图片出来，因此我们不用逐步解码；
    //如果我们直接使用 WebPDecode() 去解码多帧的 webp 图片，我们会收到一个 VP8_STATUS_UNSUPPORTED_FEATURE 的错误，因此我们需要先用 WebPDemuxer去解包
    

    //创建WebPData的结构体对象webPData
    WebPData webPData = {0};
    //用_data.bytes和length赋值于webPData的bytes和size
    webPData.bytes = _data.bytes;
    webPData.size = _data.length;
    //解析由“data”给出的完整WebP文件 对于单个图像，单独的WebP文件头或者文件头和块头可能不存在 返回一个WebPDemuxer对象
    //WebPDemuxer 等于一个分离器 将原本多帧的webp图片分离成单帧
    WebPDemuxer *demuxer = WebPDemux(&webPData);
    if (!demuxer) return;
    //从WebPDemuxer对象获取对应的值
    ...
    //如果图片帧数等于0 或者 图片宽度少于1 或者 图片高度少于1 则直接销毁WebPDemuxer对象 释放空间
    ...
    //创建WebPIterator结构体对象
    //它是用来在我们迭代每一帧时，每一帧数据显示的内容，其中包括帧数，图片的宽高，在画布（canvas）上的偏移量，动画持续时间（毫秒），混合方式，渲染方式，帧数据，是否包含alpha通道
    WebPIterator iter = {0};
    //从WebPDemuxer对象上获取第一帧 从用iter指向该帧
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _YYImageDecoderFrame *frame = [_YYImageDecoderFrame new];
            [frames addObject:frame];
            //iter.dispose_method 是指动态webp图片 是在前一帧渲染的时候下一帧以何种方法工作的
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                //在渲染下一帧的之前 先把画布清空为默认背景色
                frame.dispose = YYImageDisposeBackground;
            }
            //iter.blend_method 是指当前帧的透明像素是如何与前一个画布的像素混合的。
            if (iter.blend_method == WEBP_MUX_BLEND) {
                //该帧应该根据其alpha值合成到输出缓冲区中
    ...
    //如果帧数组里的个数 不等于 webpFrameCount 则代表循环时出错 进入下面的判断
...
- (void)_updateSourceImageIO {
    ...
    //如果_source没有值 则进入下面的判断
    if (!_source) {
        //如果已完成数据输入
        if (_finalized) {
            //通过数据 创建图片资源CGImageSourceRef对象
            _source = CGImageSourceCreateWithData((__bridge CFDataRef)_data, NULL);
        } else {
            //创建增量图片资源CGImageSourceRef对象
            _source = CGImageSourceCreateIncremental(NULL);
            //如果创建_source成功 则传输数据更新_source对象
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        //假如图片类型是YYImageTypePNG 则忽略其帧数 并用自己的解码器去解码
        if (_type == YYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        //假如图片类型是YYImageTypeGIF 则进入下面的判断
        if (_type == YYImageTypeGIF) { // get gif loop count
            //获取gif图片的循环次数kCGImagePropertyGIFLoopCount的值
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
    ...
    for (NSUInteger i = 0; i < _frameCount; i++) {
        //创建_YYImageDecoderFrame类的实例 获取部分值 并添加到frames
        _YYImageDecoderFrame *frame = [_YYImageDecoderFrame new];
        frame.index = i;
        frame.blendFromIndex = i;
        frame.hasAlpha = YES;
        ...
        if (properties) {
            ...
            //获取其Unclamped帧延迟时间DelayTime和运行时间duration
            if (_type == YYImageTypeGIF) {
                ...
                if (gif) {
                    ...
                    if (!value) {
                        ...
                        //假如unclamped frame delay不存在 则退回用clamped frame delay
            ...
            //如果刚创建的图片 那就第一帧的_width和_height等为0
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    //如果还没完成 而且 还是多帧 则返回null
    if (!_finalized && index > 0) return NULL;
    //如果已完成 可是索引大于帧数组个数 则返回null
    ...
    if (_source) {
        ...
        //若创建成功 并需要绘制到画布上 则进入下面的判断
        if (imageRef && extendToCanvas) {
            size_t width = CGImageGetWidth(imageRef);
            size_t height = CGImageGetHeight(imageRef);
            //假如图片和画布同大小
            if (width == _width && height == _height) {
                //通过判断是否需要重绘 用两种不同方法去生成图片
                CGImageRef imageRefExtended = YYCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                    imageRef = imageRefExtended;
                    //赋值decoded变量 说明已经解码完毕
                    if (decoded) *decoded = YES;
                }
            } else {
                //若需要重新绘制到画布上 则进入下面的判断
                //获取位图的上下文
                //kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst 代表是 RGB32 带有alpha通道
    ...
    if (_apngSource) {
        ...
        //使用此帧数据创建 CGDataProviderRef ，然后根据此 provider 生成 CGImageSourceRef
        ...
        //通过provider创建CGImageSourceRef结构体对象source 即图片资源
...
#if YYIMAGE_WEBP_ENABLED
    if (_webpSource) {
        //通过方法 WebPDemuxGetFrame 获取 _webpSource 第 index 的图片数据，若获取不到直接返回 NULL；获取到的数据使用 WebPIterator 接收；
        ...
        //使用 WebPDecoderConfig 来得到图片数据和像素信息；
        ...
        //然后使用 CGDataProviderCreateWithData 函数得到 CGDataProviderRef ，后面的就跟上面的操作类似了，得到 CGImageRef 并返回
...
- (CGImageRef)_newBlendedImageWithFrame:(_YYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    //YYImageDisposePrevious 指的是在渲染下一帧之前，画布的帧区域将被还原为先前一帧的内容。
    if (frame.dispose == YYImageDisposePrevious) {
        //YYImageBlendOver 帧应该根据其alpha值合成到输出缓冲区中。
        if (frame.blend == YYImageBlendOver) {
            //之前的图片内容 _blendCanvas 指就是一开始时是一张空白的内容 可是可能会被其他所用 可能会有内容
            CGImageRef previousImage = CGBitmapContextCreateImage(_blendCanvas);
            //未混合的图片内容
            CGImageRef unblendImage =  [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            //若存在未混合的图片 则进入下面的判断
            if (unblendImage) {
                //在_blendCanvas上下文里以unblendImage为内容绘制图片
                CGContextDrawImage(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
                CFRelease(unblendImage);
            }
            //生成图片
            imageRef = CGBitmapContextCreateImage(_blendCanvas);
            //清除_blendCanvas上下文的内容
            CGContextClearRect(_blendCanvas, CGRectMake(0, 0, _width, _height));
            //重新在_blendCanvas上下文里绘制之前的空白内容
            if (previousImage) {
                CGContextDrawImage(_blendCanvas, CGRectMake(0, 0, _width, _height), previousImage);
                CFRelease(previousImage);
            }
        } else {
            //那就是frame.blend == YYImageBlendNone
            //YYImageBlendNone 指的是框架的所有颜色组件（包括alpha）都会覆盖该帧画布区域的当前内容 相当于绘制前先清空画布的指定区域。
            CGImageRef previousImage = CGBitmapContextCreateImage(_blendCanvas);
            CGImageRef unblendImage = [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            if (unblendImage) {
                //清除_blendCanvas上下文里之前的内容
                CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
                //在_blendCanvas上下文里以unblendImage为内容绘制图片
                CGContextDrawImage(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
                CFRelease(unblendImage);
        ...
        //YYImageDisposeBackground 指的是在渲染下一帧之前 先把画布清空为默认背景色
    } else if (frame.dispose == YYImageDisposeBackground) {
        //YYImageBlendOver 帧应该根据其alpha值合成到输出缓冲区中 即通常情况下两张图片重叠的效果。
        if (frame.blend == YYImageBlendOver) {
        ...
        if (frame.blend == YYImageBlendOver) {
            ...
            //直接用在_blendCanvas上下文里以unblendImage为内容绘制的图片
            imageRef = CGBitmapContextCreateImage(_blendCanvas);
        }
    }
    return imageRef;
...
- (instancetype)yy_imageByDecoded {
    //整个方法其实就是调用YYCGImageCreateDecodedCopy 来返回已经解压缩的图片
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    ...
    if ([self isKindOfClass:[YYImage class]]) {
        ...
        //判断是否由动态图组成的图片数据 如果是 则进入下面的判断
        if (image.animatedImageData) {
            //如果是来自系统的 因为系统的仅支持GIF和PNG 所以要符合这两个类型的才返回animatedImageData
            if (forSystem) { // system only support GIF and PNG
                ...
                //如果不是来自系统的 就全部动态图类型都返回animatedImageData
                data = image.animatedImageData;
            }
        }
    }
    //能进入下面的判断 就是静态图 要不就是不符合系统支持的GIF和PNG的动态图类型
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                //判断现在图片旋转角度 如果不是默认的那种 就重新设置图片的旋转角度 生成新的CGImageRef对象rotated
            ...
            @autoreleasepool {
                //通过imageRef创建新的UIImage对象newImage 然后判断是否有alpha通道 若有则将图片以PNG格式返回data 若无 则以jpeg格式返回
    ...
    //若到这里 就有可能是获取不到图片的CGImage 或者转换的时候出了问题 则统一以PNG格式返回data
    if (!data) {
